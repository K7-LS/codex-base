...
def build(output: Path, root: Path = ROOT, *, for_publication: bool = False) -> dict:
    ...
    files = {
        "migrate-sync-base.ps1": (root / "migration/universal/migrate-sync-base.ps1").read_bytes().replace(b"\r\n", b"\n"),
        "connection.ps1": (root / "runtime/connection.ps1").read_bytes().replace(b"\r\n", b"\n"),
        "README.md": (root / "migration/universal/README.md").read_bytes().replace(b"\r\n", b"\n"),
    }
    ...
    archive_path = output / "sync-base-migration-1.0.1.zip"
    ...
    manifest = {
        ...
        "tag": TAG,
        "target": "codex",
        "migration_protocol": "codex-sync-migration-v2",
        "accepted_protocol": "installation-integrity-v1",
        "base_release": "latest immutable stable codex-vX.Y.Z, at least 0.2.3",
