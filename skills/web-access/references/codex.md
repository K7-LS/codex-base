...
`next`), у Codex следующий шаг — MCP exa/firecrawl/playwright. У Codex этих
...
| Ступень после web_get | Codex | Codex |
|---|---|---|
...
тяжёлые SPA) — тот же случай, для которого у Codex был playwright с
...
- Основной `SKILL.md` — детерминированные ступени, верификация, гео-модель,
  без изменений.
- `~/.codex/base/codex-layer/mcp-whitelist.json` — состав MCP, доступных Codex
  (сейчас `time`; список предварительный до тест-заезда Задачи 11).
- [[ru-gov-access]] — RU-слой, общий для Codex и Codex.
