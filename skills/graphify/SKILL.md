...
У graphify 40+ команд/флагов, и три из них легко жгут токены или ломают Codex. Перед запуском:
...
3. **Не ставь ломающий хук вслепую.** `graphify Codex install` вшивает PreToolUse-хук, который на
   части сборок Codex слетает и **блокирует чтение файлов**. НЕ ставить по своей инициативе;
   для авто-обновления рекомендуй безопасный **post-commit** (`graphify hook install`). Откат, если
   хук уже сломал чтение: удалить блок `hooks` с `graphify` из `settings.json` → перезапустить Codex.
...
> **Even on the fast path, do NOT skip vocab-expansion.** `## For /graphify query` begins with a REQUIRED Step 0 (constrained vocab-expansion, see `references/query.md`). graphify's matcher has no stemming/synonyms/cross-language — a Russian question against English or normalized labels (our Codex-base case) returns **0 hits** without expansion. Expand first, then query. "Run immediately" means "don't rebuild", not "skip expansion".
...
> (gitignore-синтаксис) в корне сканирования — наряду с `.gitignore`. Для графа Codex-base такой файл
> уже лежит в `~/.codex/base/.graphifyignore` и исключает session-reports/harvested/evals/служебные каталоги:
...
> **Windows hub / Codex-base:** on a Windows host with a Cyrillic install path, do NOT
