...
шаг 6 программы «Усиление Codex в Revit»).
...
- **Этот форк:** самостоятельная промпт-обёртка над НАШИМ executor'ом
  `<проект>/Codex/famfab/gen/` (схема v2, метрическая, RU, живой стенд Revit 2025 +
...
Требует проект с модулем `Codex/famfab/gen/` и живой стенд — без них скилл только
