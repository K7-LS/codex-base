...
propose: read-only сверка STATUS.md с журналом и реальностью файлов →
         Codex/.curate/<stamp>/{proposals.json, REPORT.md}
apply:   бэкап затрагиваемых файлов в Codex/_backup_<дата_время>/ →
         применение ТОЛЬКО явно принятых предложений (--accept id1,id2).
         Скрипт сам никогда не решает, что применять: апрув ведёт
         человек/Codex (см. SKILL.md и prompts/rot.md).
...
не пишет вне Codex/. git log сознательно не используется: папки объектов
...
ARCHIVE_REL = Path("Codex") / "_АРХИВ" / "из-курирования.md"
...
def find_project_root(start: Path) -> Path | None:
    """Идёт вверх от start (≤12 уровней) до папки с Codex/ЖУРНАЛ СЕССИЙ.md."""
    d = Path(start).resolve()
    for _ in range(12):
        if (d / "Codex" / JOURNAL_NAME).exists():
            return d
        if d.name == "Codex" and (d / JOURNAL_NAME).exists():
            return d.parent
        if d.parent == d:
            return None
        d = d.parent
...
def _scan_status(root: Path, col: "_Collector", today: date) -> None:
    status_path = root / "Codex" / STATUS_NAME
    if not status_path.exists():
        return
    target = "Codex/" + STATUS_NAME
    ...
    journal = root / "Codex" / JOURNAL_NAME
...
def propose(start: Path) -> Path:
    """Read-only анализ. Возвращает папку Codex/.curate/<stamp>/."""
    root = find_project_root(Path(start))
    if root is None:
        raise SystemExit(
            f"память проекта не найдена (нет Codex/{JOURNAL_NAME} вверх от {start})")
    today = date.today()
    col = _Collector()
    _scan_status(root, col, today)

    stamp = datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    out_dir = root / "Codex" / ".curate" / stamp
    n = 2
    while out_dir.exists():
        out_dir = root / "Codex" / ".curate" / f"{stamp}-{n}"
        n += 1
...
def _render_report(payload: dict) -> str:
    ...
    lines = [
        ...
        "(бэкап в `Codex/_backup_<дата>/` создаётся автоматически; "
...
def apply(start: Path, stamp: str, accept: list[str]) -> dict:
    ...
    (через Codex/request_user_input) на review-шаге."""
    root = find_project_root(Path(start))
    if root is None:
        raise SystemExit("память проекта не найдена")
    cur_dir = root / "Codex" / ".curate" / stamp
    pfile = cur_dir / "proposals.json"
    if not pfile.exists():
        raise SystemExit(
            f"нет прогона propose: Codex/.curate/{stamp}/proposals.json")
    ...
    if touched:
        ...
        backup_dir = root / "Codex" / f"_backup_{ts}"
        n = 2
        while backup_dir.exists():
            backup_dir = root / "Codex" / f"_backup_{ts}-{n}"
            n += 1
        ...
        res["backup"] = "Codex/" + backup_dir.name
    ...
    for a in accept:
        ...
        if not target.startswith("Codex/"):
            res["errors"].append(f"{pid}: target вне Codex/ запрещён ({target})")
            continue
        if p["action"] == "flag":
...
def main(argv=None) -> int:
    ...
    ap.add_argument("stamp", help="метка прогона propose (папка в Codex/.curate/)")
