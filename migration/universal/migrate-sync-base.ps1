...
# Stable-release protocol migration. Verify this bridge release and its assets
# BEFORE executing this script; see the included README.md.
Set-StrictMode -Version 2.0
$ErrorActionPreference = 'Stop'
$script:MigrationRepository = 'K7-LS/codex-base'
$script:MigrationTag = ''
$script:MigrationVersion = ''
...
function Assert-MigrationLatestRelease {
    $json = Invoke-MigrationGh -Arguments @('api', '--paginate', '--slurp',
        ('repos/' + $script:MigrationRepository + '/releases?per_page=100'))
    try { $pages = $json | ConvertFrom-Json }
    catch { throw 'GitHub release discovery returned invalid JSON.' }
    $candidates = @()
    foreach ($page in $pages) {
        foreach ($release in $page) {
            if ($release.tag_name -cmatch '^codex-v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$' -and
                $release.draft -eq $false -and $release.prerelease -eq $false) {
                $candidates += $release
            }
        }
    }
    $candidates = @($candidates | Sort-Object { [version]($_.tag_name.Substring(7)) } -Descending)
    if ($candidates.Count -eq 0 -or $candidates[0].immutable -ne $true) {
        throw 'Latest Codex Base release is not immutable stable.'
    }
    $version = [version]($candidates[0].tag_name.Substring(7))
    if ($version -lt [version]'0.2.3') {
        throw 'Latest stable predates the client-independent release contract.'
    }
    $script:MigrationTag = [string]$candidates[0].tag_name
    $script:MigrationVersion = $version.ToString()
}
...
function Export-MigrationUpdater {
    ...
    if ($manifest.target -cne 'codex' -or $manifest.version -cne $script:MigrationVersion -or
        $manifest.tag -cne $script:MigrationTag -or $manifest.channel -cne 'stable' -or
        $manifest.client.id -cne 'codex-cli' -or
        $manifest.requires.immutable_release -ne $true -or
        $manifest.requires.release_attestation -ne $true) {
        throw 'Migration release manifest target/version/channel differs.'
    }
    $packageName = 'codex-base-' + $script:MigrationVersion + '.zip'
    $sessionName = 'session-tools-codex-' + $script:MigrationVersion + '.zip'
    $bindings = @(
        [pscustomobject]@{name=$packageName; sha=[string]$manifest.asset.sha256; bytes=$manifest.asset.bytes},
        [pscustomobject]@{name=$sessionName; sha=[string]$manifest.session_tools_asset.sha256; bytes=$manifest.session_tools_asset.bytes},
        [pscustomobject]@{name='components.lock.json'; sha=[string]$manifest.components_lock_sha256; bytes=$null},
        [pscustomobject]@{name='acceptance-evidence.json'; sha=[string]$manifest.acceptance_evidence_sha256; bytes=$null}
    )
    if ([string]$manifest.asset.name -cne $packageName -or
        [string]$manifest.session_tools_asset.name -cne $sessionName) {
        throw 'Migration release asset names differ.'
    }
    foreach ($binding in $bindings) {
        $path = Join-Path $AssetDirectory $binding.name
        if ($binding.sha -cnotmatch '^[a-f0-9]{64}$' -or
            -not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Migration asset binding is invalid: $($binding.name)"
        }
        $bytes = [IO.File]::ReadAllBytes($path)
        if (($null -ne $binding.bytes -and $bytes.Length -ne $binding.bytes) -or
            (Get-MigrationHash $bytes) -cne $binding.sha) {
            throw "Migration asset bytes differ before updater load: $($binding.name)"
        }
    }
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $archive = [IO.Compression.ZipFile]::OpenRead((Join-Path $AssetDirectory $packageName))
    $payloads = [ordered]@{}
    try {
...
function Convert-MigrationPlanOutput {
    ...
    $manifestPath = Join-Path (Split-Path -Parent $Verified.asset_path) 'release-manifest.json'
    $manifest = Get-Content -LiteralPath $manifestPath -Raw -Encoding UTF8 | ConvertFrom-Json
    $expectedHash = [string]$manifest.asset.sha256
    if ($expectedHash -cnotmatch '^[a-f0-9]{64}$' -or
        [string]$manifest.tag -cne $script:MigrationTag -or
        [string]$manifest.version -cne $script:MigrationVersion) {
        throw 'Verified migration release binding differs.'
    }
    ...
    if ($plan.status -cne 'READY' -or $plan.target -cne 'codex' -or
        ...
        $plan.client.id -cne 'codex-cli' -or
        $plan.client.supported_version -cne [string]$Verified.client_version -or
...
function Invoke-SyncMigration {
    ...
    Invoke-WithLlmConnection -HomePath $targetRoot -ScriptBlock {
        ...
        $expected = @(
            ...
            ('codex-base-' + $script:MigrationVersion + '.zip'),
            'components.lock.json',
            'release-manifest.json',
            ('session-tools-codex-' + $script:MigrationVersion + '.zip')
        ) | Sort-Object
    ...
    if ($Mode -ceq 'Verify') {
        [pscustomobject]@{status='VERIFIED'; installation='NOT_RUN'; target='codex'; tag=$script:MigrationTag;
            acceptance_protocol='installation-integrity-v1'; installed_version=$installed;
            required_client_id='codex-cli'; client_version_policy='ANY_VALID_VERSION'} | ConvertTo-Json
        return
    }
